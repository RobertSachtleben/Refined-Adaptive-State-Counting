...
fun m_traversal_paths_with_witness_up_to_length :: "('a,'b,'c) fsm \<Rightarrow> 'a \<Rightarrow> ('a set \<times> 'a set) list \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> (('a\<times>'b\<times>'c\<times>'a) list \<times> ('a set \<times> 'a set)) set" where
...
fun m_traversal_paths_with_witness :: "('a,'b,'c) fsm \<Rightarrow> 'a \<Rightarrow> ('a set \<times> 'a set) list \<Rightarrow> nat \<Rightarrow> (('a\<times>'b\<times>'c\<times>'a) list \<times> ('a set \<times> 'a set)) set" where
...
lemma m_traversal_paths_with_witness_up_to_length_max_length :
  ...
  and     "q \<in> nodes M"
  and     "(p,d) \<in> set (m_traversal_paths_with_witness_up_to_length M q D m k)"
...
proof (rule ccontr)
  ...
  have "path M q p"
    ...
    using paths_up_to_length_or_condition_with_witness_path_set_nil[OF assms(3), of k ?f] assms(4) by auto
...
lemma m_traversal_paths_with_witness_set :
  ...
  and     "\<forall> d \<in> set D . snd d \<subseteq> fst d"
  and     "q \<in> nodes M"
  shows "set (m_traversal_paths_with_witness M q D m) = {(p,d) | p d . path M q p \<and> find (\<lambda>d. Suc (m - card (snd d)) \<le> length (filter (\<lambda>t. t_target t \<in> fst d) p)) D = Some d
...
proof -
  ...
  have "\<And> p . p \<in> ?MTP \<Longrightarrow> p \<in> ?P"
    using paths_up_to_length_or_condition_with_witness_path_set_nil[of  q M "(Suc (size M * m))" ?f, OF assms(3)]
    unfolding m_traversal_paths_with_witness.simps m_traversal_paths_with_witness_up_to_length.simps by force
  ...
  proof -
    ...
    then have "(p,x) \<in> set (m_traversal_paths_with_witness_up_to_length M q D m (length p))"
      using paths_up_to_length_or_condition_with_witness_path_set_nil[of q M "length p" ?f, OF assms(3)] 
      unfolding m_traversal_paths_with_witness_up_to_length.simps by force
    ...
    have "(p,x) \<in> ?MTP"
      ...
      using paths_up_to_length_or_condition_with_witness_path_set_nil[of q M "(Suc (size M * m))" ?f]  
      unfolding m_traversal_paths_with_witness.simps m_traversal_paths_with_witness_up_to_length.simps 
...
fun N :: "('a,'b,'c) fsm \<Rightarrow> 'a \<Rightarrow> ('a set \<times> 'a set) set \<Rightarrow> nat \<Rightarrow> Input list list" where
...
fun T :: "('a,'b,'c) fsm \<Rightarrow> 'a \<Rightarrow> ('a set \<times> 'a set) set \<Rightarrow> nat \<Rightarrow> (Input \<times> Output) list list" where
  "T M q D m = add_prefixes (map (\<lambda> p . p_io p) (m_traversal_paths M q D m))"

value "remdups (T M_ex_H 1 {({1,3,4},{1,3,4}),({2,3,4},{3,4})} 4)" 

fun Traces :: "('a,'b,'c) fsm \<Rightarrow> 'a \<Rightarrow> ('a set \<times> 'a set) set \<Rightarrow> nat \<Rightarrow> Input list \<Rightarrow> (Input \<times> Output) list list" where
...
definition Traces_not_to_cut :: "('a,'b,'c) fsm \<Rightarrow> 'a \<Rightarrow> ('a set \<times> 'a set) set \<Rightarrow> nat \<Rightarrow> Input list \<Rightarrow> (Input \<times> Output) list list" where
...
definition Traces_to_cut :: "('a,'b,'c) fsm \<Rightarrow> 'a \<Rightarrow> ('a set \<times> 'a set) set \<Rightarrow> nat \<Rightarrow> Input list \<Rightarrow> (Input \<times> Output) list list" where
...
definition Traces_simplified :: "('a,'b,'c) fsm \<Rightarrow> 'a \<Rightarrow> ('a set \<times> 'a set) set \<Rightarrow> nat \<Rightarrow> Input list \<Rightarrow> (Input \<times> Output) list list" where
...
(* N - helper *)
(*
fun m_traversal_sequences' :: "('a,'b,'c) fsm \<Rightarrow> 'a \<Rightarrow> ('a set \<times> 'a set) set \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> Input list set \<Rightarrow> Input list set \<Rightarrow> Input list set" where
...
(* TODO: extremely slow *)
fun m_traversal_sequences_list :: "('a,'b,'c) fsm \<Rightarrow> 'a \<Rightarrow> ('a set \<times> 'a set) set \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> Input list list" where
...
(* N *)
fun m_traversal_sequences :: "('a,'b,'c) fsm \<Rightarrow> 'a \<Rightarrow> 'a set set \<Rightarrow> nat \<Rightarrow> Input list set" where
