...
proof -
  ...
  obtain p2 where "path M (target q p1) p2" and "length p2 = (Suc ((size M) * m)) - length p1"
    using path_of_length_ex[OF assms(1) path_target_is_node[OF assms(6)] assms(3)] 
    by blast
  ...
  have "\<exists> q \<in> nodes M . length (filter (\<lambda>t . t_target t = q) (p1@p2)) \<ge> Suc m"
  proof (rule ccontr)
    assume "\<not> (\<exists>q\<in>nodes M. Suc m \<le> length (filter (\<lambda>t. t_target t = q) (p1@p2)))"
    then have "\<forall> q \<in> nodes M . length (filter (\<lambda>t. t_target t = q) (p1@p2)) < Suc m"
      by auto
    then have "\<forall> q \<in> nodes M . length (filter (\<lambda>t. t_target t = q) (p1@p2)) \<le> m"
      by auto
    
    have "(\<Sum>q\<in>nodes M. length (filter (\<lambda>t. t_target t = q) (p1@p2))) \<le> (\<Sum>q\<in>nodes M . m)"
      using \<open>\<forall> q \<in> nodes M . length (filter (\<lambda>t. t_target t = q) (p1@p2)) \<le> m\<close> by (meson sum_mono) 
  ...
  show ?thesis proof (cases "(\<forall>p' p''. (p1@p2) = p' @ p'' \<and> p'' \<noteq> [] \<longrightarrow> find (\<lambda>d. Suc (m - card (snd d)) \<le> length (filter (\<lambda>t. t_target t \<in> fst d) p')) D = None)")
    case True
    then show ?thesis using m_traversal_paths_with_witness_set[OF assms(4,5,2), of m] \<open>path M q (p1@p2)\<close> \<open>?f (p1@p2) = Some d'\<close> by blast
  next
    case False

    define ps where ps_def: "ps = {p' . \<exists> p''. (p1@p2) = p' @ p'' \<and> p'' \<noteq> [] \<and> find (\<lambda>d. Suc (m - card (snd d)) \<le> length (filter (\<lambda>t. t_target t \<in> fst d) p')) D \<noteq> None}"
    have "ps \<noteq> {}" using False ps_def by blast
    moreover have "finite ps" 
    proof -
...
(* TODO: combine with m_traversal_path_extension_exist *)
...
proof -
  ...
  obtain p2 where "path M ?q p2" and "length p2 = (Suc ((size M) * m)) - (length ?p)"
    using path_of_length_ex[OF assms(1) path_target_is_node[OF \<open>path M q (p1@[t])\<close>] assms(3)] 
    by blast
  ...
  have "\<exists> q \<in> nodes M . length (filter (\<lambda>t . t_target t = q) (?p@p2)) \<ge> Suc m"
  proof (rule ccontr)
    assume "\<not> (\<exists>q\<in>nodes M. Suc m \<le> length (filter (\<lambda>t. t_target t = q) (?p@p2)))"
    then have "\<forall> q \<in> nodes M . length (filter (\<lambda>t. t_target t = q) (?p@p2)) < Suc m"
      by auto
    then have "\<forall> q \<in> nodes M . length (filter (\<lambda>t. t_target t = q) (?p@p2)) \<le> m"
      by auto
    
    have "(\<Sum>q\<in>nodes M. length (filter (\<lambda>t. t_target t = q) (?p@p2))) \<le> (\<Sum>q\<in>nodes M . m)"
      using \<open>\<forall> q \<in> nodes M . length (filter (\<lambda>t. t_target t = q) (?p@p2)) \<le> m\<close> by (meson sum_mono) 
  ...
  next
    ...
    next
      ...
      define ps where ps_def: "ps = {p' . \<exists> p''. (?p@p2) = p' @ p'' \<and> p'' \<noteq> [] \<and> find (\<lambda>d. Suc (m - card (snd d)) \<le> length (filter (\<lambda>t. t_target t \<in> fst d) p')) D \<noteq> None}"
      have "ps \<noteq> {}" using False ps_def by blast
      moreover have "finite ps" 
      proof -
