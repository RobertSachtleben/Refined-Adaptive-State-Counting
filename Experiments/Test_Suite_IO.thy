...
proof 
  ...
  then obtain RepSets where RepSets_def: "is_sufficient_for_reduction_testing_for_RepSets (Test_Suite prs tps rd_targets atcs) M m RepSets"
    using assms(1) unfolding is_sufficient_for_reduction_testing_def 
    by blast
  then have "is_sufficient_for_reduction_testing (Test_Suite prs tps rd_targets atcs) M m"
    unfolding is_sufficient_for_reduction_testing_def 
    by blast
  ...
  next
    ...
    have "ioA \<in> LS M (target q pt)"
      using from_FSM_language[OF \<open>(target q pt) \<in> nodes M\<close>] \<open>ioA \<in> L (from_FSM M (target q pt))\<close>
      by blast
...
proof -
  ...
  proof -
    have "\<exists>ps. (butlast ioF' @ [last ioF']) @ ps = ioF"
      using \<open>ioF' = butlast ioF' @ [last ioF']\<close> \<open>ioF' \<in> {ioF' \<in> set (prefixes ioF). ioF' \<in> L M' \<and> ioF' \<notin> L M}\<close> prefixes_set 
      by auto
    then show ?thesis
  ...
  proof -
    ...
    then show ?thesis
      using that \<open>completely_specified M\<close> path_target_is_node[OF \<open>path M (initial M) p\<close>] 
  ...
  proof -
    ...
    proof (rule ccontr)
      ...
      next
        ...
        proof -
          ...
          have **: "([?t] @ (drop (length (ioT @ [(x,y)])) pT) @ pT') \<noteq> []"
            by simp
          show ?thesis 
            using \<open>\<And> p' p''. (pT @ pT') = p' @ p'' \<Longrightarrow> p'' \<noteq> [] \<Longrightarrow> find (\<lambda>d. Suc (m - card (snd d)) \<le> length (filter (\<lambda>t. t_target t \<in> fst d) p')) RepSets = None\<close>[OF * **] 
            by assumption
        qed
        ...
        obtain pTt' dt' where "(?p @ [t'] @ pTt', dt') \<in> m_traversal_paths_with_witness M q RepSets m"
          using m_traversal_path_extension_exist_for_transition[OF \<open>completely_specified M\<close> \<open>q \<in> nodes M\<close> \<open>FSM.inputs M \<noteq> {}\<close> 
                                                                   t5 t8 \<open>path M q ?p\<close> \<open>find (\<lambda>d. Suc (m - card (snd d)) \<le> length (filter (\<lambda>t. t_target t \<in> fst d) ?p)) RepSets = None\<close> 
                                                                   \<open>t' \<in> transitions M\<close> \<open>t_source t' = target q ?p\<close>]
          by blast

        have "?p @ [t'] @ pTt' \<in> tps q"
          using t6[OF \<open>q \<in> fst ` prs\<close> ] \<open>(?p @ [t'] @ pTt', dt') \<in> m_traversal_paths_with_witness M q RepSets m\<close>
          by force
      ...
      next
        ...
        have "q \<in> nodes M"
          using is_preamble_is_node[OF t2[OF \<open>(q, P) \<in> prs\<close>]] 
          by assumption
        ...
        have "qT \<in> nodes M'"
          using \<open>qT \<in> io_targets M' (p_io pP @ p_io pT) (FSM.initial M')\<close> io_targets_nodes subset_iff 
          by fastforce 
        ...
        have "\<not> pass_io_set (FSM.from_FSM M' qT) (atc_to_io_set (FSM.from_FSM M (target q pT)) A)"
          using \<open>\<not>pass_separator_ATC M' A qT d2\<close>
                pass_separator_pass_io_set_iff[OF \<open>is_separator M (target q pT) q' A d1 d2\<close> \<open>observable M\<close> 
                                                  \<open>observable M'\<close> path_target_is_node[OF \<open>path M q pT\<close>] 
                                                  \<open>qT \<in> nodes M'\<close> \<open>inputs M' = inputs M\<close> \<open>completely_specified M\<close>]
          by simp
        ...
        proof -
          ...
          proof -
            ...
            then have "io @ [(x, y')] \<in> L (FSM.from_FSM M (target q pT))"
              unfolding from_FSM_language[OF path_target_is_node[OF \<open>path M q pT\<close>]] 
              by assumption
        ...
        next
          ...
          have "pass_io_set (FSM.from_FSM M' (target (FSM.initial M') pt')) (atc_to_io_set (FSM.from_FSM M (target q pt)) A)"
            using  pass_io_set_from_pass_separator[OF \<open>is_separator M (target q pt) q' A t1 t2\<close> 
                                                      ...
                                                      \<open>observable M\<close> \<open>observable M'\<close> path_target_is_node[OF \<open>path M q pt\<close>] 
                                                      \<open>target (FSM.initial M') pt' \<in> FSM.nodes M'\<close> \<open>inputs M' = inputs M\<close>]
            by assumption
          ...
          moreover have "ioAI @ [(x, y')] \<in> L (FSM.from_FSM M' (target (FSM.initial M') pt'))" 
            ...
            by (metis (no_types, lifting) \<open>target (FSM.initial M') pt' \<in> FSM.nodes M'\<close> 
...
fun test_suite_to_io'_maximal :: "('a::linorder,'b::linorder,'c) fsm \<Rightarrow> ('a,'b,'c,'d::linorder) test_suite \<Rightarrow> ('b \<times> 'c) list set" where
