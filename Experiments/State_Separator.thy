...
abbreviation(input) distinguishing_transitions_left :: "('a, 'b, 'c) fsm \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> (('a \<times> 'a + 'a) \<times> 'b \<times> 'c \<times> ('a \<times> 'a + 'a)) set" where
...
abbreviation(input) distinguishing_transitions_right :: "('a, 'b, 'c) fsm \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> (('a \<times> 'a + 'a) \<times> 'b \<times> 'c \<times> ('a \<times> 'a + 'a)) set" where
...
abbreviation(input) distinguishing_transitions_left_alt :: "('a, 'b, 'c) fsm \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> (('a \<times> 'a + 'a) \<times> 'b \<times> 'c \<times> ('a \<times> 'a + 'a)) set" where
...
abbreviation(input) distinguishing_transitions_right_alt :: "('a, 'b, 'c) fsm \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> (('a \<times> 'a + 'a) \<times> 'b \<times> 'c \<times> ('a \<times> 'a + 'a)) set" where
...
lemma distinguishing_transitions_left_alt_def :
...
proof -
  ...
  moreover have "\<And> t . t \<in> distinguishing_transitions_left_alt M q1 q2 \<Longrightarrow> t \<in> distinguishing_transitions_left M q1 q2"
    by fastforce
  ultimately show ?thesis
    by (metis (no_types, lifting) Collect_cong mem_Collect_eq) 
qed

lemma distinguishing_transitions_right_alt_def :
...
proof -
  ...
  have "t \<in> ?shftd \<union> ?dl \<union> ?dr"
    using assms(1) unfolding canonical_separator_transitions_alt_def[OF assms(2,3)] by force
...
lemma distinguishing_transitions_left_sources_targets :
  assumes "t \<in> (distinguishing_transitions_left_alt M q1 q2)"
    ...
    obtains q1' q2' t' where "t_source t = Inl (q1',q2')" 
                            ...
                            "t_target t = Inr q1"
  using assms(1) assms(2) fsm_transition_source path_target_is_node by fastforce
...
lemma shifted_transitions_underlying_transition :
  assumes "tS \<in> (shifted_transitions (transitions (restrict_to_reachable_nodes (product (from_FSM M q1) (from_FSM M q2)))))"
  and     "q1 \<in> nodes M"
  and     "q2 \<in> nodes M"
...
proof -
  obtain t where "tS = (Inl (t_source t), t_input t, t_output t, Inl (t_target t))"
           and   *: "t \<in> (transitions (restrict_to_reachable_nodes (product (from_FSM M q1) (from_FSM M q2))))"
    using assms unfolding shifted_transitions_def by blast
...
end (* lemma distinguishing_transitions_left_underlying_data :
...
end (* lemma distinguishing_transitions_right_underlying_data :
...
end (* lemma shifted_transitions_observable_against_distinguishing_transitions_left :
...
end (* lemma shifted_transitions_observable_against_distinguishing_transitions_right :
  assumes "t1 \<in> set (shifted_transitions M q1 q2)"
...
end (* lemma distinguishing_transitions_left_observable_against_distinguishing_transitions_right :
...
proof 
  ...
  then have ****: "snd qqtL \<in> (transitions M) \<and> t_source (snd qqtL) = fst (fst qqtL) \<and> t_input (snd qqtL) = t_input (snd qqtL) \<and> t_output (snd qqtL) = t_output (snd qqtL)"
    using *** by auto
...
end (* lemma distinguishing_transitions_left_observable_against_distinguishing_transitions_left :
  assumes "t1 \<in> set (distinguishing_transitions_left M q1 q2)"
  and     "t2 \<in> set (distinguishing_transitions_left M q1 q2)"
  and     "q1 \<in> nodes M"
  and     "q2 \<in> nodes M"
...
end (* lemma distinguishing_transitions_right_observable_against_distinguishing_transitions_right :
...
end (* lemma shifted_transitions_observable_against_shifted_transitions :
  assumes "t1 \<in> set (shifted_transitions M q1 q2)"
...
end (* lemma canonical_separator_observable :
...
proof -
  ...
  proof -
    ...
    proof -
      have "\<forall>p. (p \<in> set (distinguishing_transitions_left M q1 q2 @ distinguishing_transitions_right M q1 q2) \<or> p \<in> set (shifted_transitions M q1 q2)) \<or> p \<notin> (transitions (canonical_separator M q1 q2))"
      ...
      then have "t1 = t2"
        by (metis (no_types) \<open>\<And>t2 t1. \<lbrakk>t1 \<in> set (distinguishing_transitions_left M q1 q2); t2 \<in> set (distinguishing_transitions_left M q1 q2); t_source t1 = t_source t2 \<and> t_input t1 = t_input t2 \<and> t_output t1 = t_output t2\<rbrakk> \<Longrightarrow> t1 = t2\<close> \<open>\<And>t2 t1. \<lbrakk>t1 \<in> set (distinguishing_transitions_left M q1 q2); t2 \<in> set (distinguishing_transitions_right M q1 q2)\<rbrakk> \<Longrightarrow> \<not> (t_source t1 = t_source t2 \<and> t_input t1 = t_input t2 \<and> t_output t1 = t_output t2)\<close> \<open>\<And>t2 t1. \<lbrakk>t1 \<in> set (distinguishing_transitions_right M q1 q2); t2 \<in> set (distinguishing_transitions_right M q1 q2); t_source t1 = t_source t2 \<and> t_input t1 = t_input t2 \<and> t_output t1 = t_output t2\<rbrakk> \<Longrightarrow> t1 = t2\<close> \<open>\<And>t2 t1. \<lbrakk>t1 \<in> set (shifted_transitions M q1 q2); t2 \<in> set (distinguishing_transitions_left M q1 q2)\<rbrakk> \<Longrightarrow> \<not> (t_source t1 = t_source t2 \<and> t_input t1 = t_input t2 \<and> t_output t1 = t_output t2)\<close> \<open>\<And>t2 t1. \<lbrakk>t1 \<in> set (shifted_transitions M q1 q2); t2 \<in> set (distinguishing_transitions_right M q1 q2)\<rbrakk> \<Longrightarrow> \<not> (t_source t1 = t_source t2 \<and> t_input t1 = t_input t2 \<and> t_output t1 = t_output t2)\<close> \<open>\<And>t2 t1. \<lbrakk>t1 \<in> set (shifted_transitions M q1 q2); t2 \<in> set (shifted_transitions M q1 q2); observable M; t_source t1 = t_source t2 \<and> t_input t1 = t_input t2 \<and> t_output t1 = t_output t2\<rbrakk> \<Longrightarrow> t1 = t2\<close> \<open>t1 \<in> (transitions (canonical_separator M q1 q2))\<close> \<open>t2 \<in> (transitions (canonical_separator M q1 q2))\<close> \<open>t_source t1 = t_source t2 \<and> t_input t1 = t_input t2 \<and> t_output t1 = t_output t2\<close> assms(1) list_concat_non_elem)
      then show ?thesis
        by meson
    qed 
  qed
...
end (* lemma distinguishing_transitions_left_empty : 
  assumes "observable M"
  and     "q \<in> nodes M"
...
proof (rule ccontr)
  ...
  then have "\<not> (\<exists>t'\<in>(transitions M).
...
end (* lemma distinguishing_transitions_right_empty : 
  assumes "observable M"
  and     "q \<in> nodes M"
...
end (* lemma canonical_separator_targets_observable_eq :
  assumes "observable M"
      and "q \<in> nodes M"
...
end (* lemma canonical_separator_targets_ineq :
  assumes "t \<in> transitions (canonical_separator M q1 q2)" 
...
proof -
  let ?C = "(canonical_separator M q1 q2)"
  have *: "(transitions ?C) = (set (shifted_transitions M q1 q2)) \<union> (set (distinguishing_transitions_left M q1 q2)) \<union> (set (distinguishing_transitions_right M q1 q2))"
...
end (* lemma canonical_separator_targets_observable :
  assumes "t \<in> transitions (canonical_separator M q1 q2)" 
...
proof -
  ...
  proof (cases "q1 = q2")
    case True
    then have "t \<in> transitions (canonical_separator M q1 q1)"
      using assms(1) by simp
    
    show ?thesis using canonical_separator_targets_observable_eq[OF assms(2,3) \<open>t \<in> transitions (canonical_separator M q1 q1)\<close>]
      using True by auto 
  next
    case False
...
end (* lemma canonical_separator_maximal_path_distinguishes_left :
...
next
  ...
  then have "\<not> (\<exists>t'\<in> transitions M. t_source t' = target (right_path pC) q2 \<and> t_input t' = t_input t \<and> t_output t' = t_output t)"
  ...
  have "p_io (right_path pC) @  [(t_input t, t_output t)] \<notin> LS M q2"
    using observable_path_language_step[OF assms(4) \<open>path M q2 (right_path pC)\<close> \<open>\<not> (\<exists>t'\<in> transitions M. t_source t' = target (right_path pC) q2 \<and> t_input t' = t_input t \<and> t_output t' = t_output t)\<close>] by assumption
...
end (* lemma canonical_separator_maximal_path_distinguishes_right :
  assumes "is_state_separator_from_canonical_separator (canonical_separator M q1 q2) q1 q2 S" (is "is_state_separator_from_canonical_separator ?C q1 q2 S")
      and "path S (initial S) p"
...
next
  ...
  then have "\<not> (\<exists>t'\<in> transitions M. t_source t' = target (left_path pC) q1 \<and> t_input t' = t_input t \<and> t_output t' = t_output t)"
  ...
  have "p_io (left_path pC) @  [(t_input t, t_output t)] \<notin> LS M q1"
    using observable_path_language_step[OF assms(4) \<open>path M q1 (left_path pC)\<close> \<open>\<not> (\<exists>t'\<in> transitions M. t_source t' = target (left_path pC) q1 \<and> t_input t' = t_input t \<and> t_output t' = t_output t)\<close>] by assumption
  moreover have "p_io p' = p_io (left_path pC)"
    using \<open>p' = map shift_Inl pC\<close> by auto
...
end (* lemma state_separator_from_canonical_separator_observable :
  assumes "is_state_separator_from_canonical_separator (canonical_separator M q1 q2) q1 q2 A"
  and     "observable M"
  and     "q1 \<in> nodes M"
  and     "q2 \<in> nodes M"
...
end (* lemma canonical_separator_initial :
  shows "initial (canonical_separator M q1 q2) = Inl (q1,q2)" 
...
end (* lemma state_separator_from_canonical_separator_initial :
  assumes "is_state_separator_from_canonical_separator (canonical_separator M q1 q2) q1 q2 A"
shows "initial A = Inl (q1,q2)"
...
end (* lemma canonical_separator_target_possibilities :
...
end (* lemma canonical_separator_nodes :
...
proof -
  ...
  consider (a) "Inl (s1,s2) = initial ?C" |
           (b) "\<exists> t \<in> transitions ?C . t_target t = Inl (s1,s2)"
  ...
  next
    case b
    then obtain t where "t \<in> transitions ?C" and "t_target t = Inl (s1,s2)"
      by blast
    then have "isl (t_target t)"
      by auto

    note State_Separator.canonical_separator_product_h_isl[OF \<open>t \<in> transitions ?C\<close> \<open>isl (t_target t)\<close>]

    show ?thesis 
      using State_Separator.canonical_separator_product_h_isl[OF \<open>t \<in> transitions ?C\<close> \<open>isl (t_target t)\<close>]
...
end (* lemma canonical_separator_transition :
  assumes "t \<in> transitions (canonical_separator M q1 q2)" (is "t \<in> transitions ?C")
  and     "q1 \<in> nodes M"
  and     "q2 \<in> nodes M"
...
proof -           
  ...
  then have "?P1 \<and> ?P2 \<and> ?P3" proof cases
    ...
    then obtain ta where **: "t = (Inl (t_source ta), t_input ta, t_output ta, Inl (t_target ta))"
                   and   ***: "ta \<in> (transitions (product (from_FSM M q1) (from_FSM M q2)))"
                   and   "\<exists>t'\<in>(transitions M).
    ...
    then have ****: "((s1,s2),t_input t, t_output t, (s1',s2')) \<in> transitions (product (from_FSM M q1) (from_FSM M q2))"
      using *** by simp
...
end (* lemma canonical_separator_transition_ex :
  assumes "t \<in> transitions (canonical_separator M q1 q2)" (is "t \<in> transitions ?C")
  and     "q1 \<in> nodes M"
  and     "q2 \<in> nodes M"
  and     "t_source t = Inl (s1,s2)"
...
end (* lemma canonical_separator_path_split_target_isl :
...
proof -
  ...
  then show ?thesis 
    using canonical_separator_t_source_isl[of t] \<open>t \<in> transitions ?C\<close> assms
    by fastforce
qed
...
end (* lemma canonical_separator_path_initial :
  assumes "path (canonical_separator M q1 q2) (initial (canonical_separator M q1 q2)) p" (is "path ?C (initial ?C) p")
  and     "q1 \<in> nodes M"
  and     "q2 \<in> nodes M"
  and     "observable M"
shows "\<And> s1' s2' . target (initial (canonical_separator M q1 q2)) p = Inl (s1',s2') \<Longrightarrow> (\<exists> p1 p2 . path M q1 p1 \<and> path M q2 p2 \<and> p_io p1 = p_io p2 \<and> p_io p1 = p_io p \<and> target p1 q1 = s1' \<and> target p2 q2 = s2')"
...
proof -
  ...
  let ?P1 = "\<forall> s1' s2' . target (initial (canonical_separator M q1 q2)) p = Inl (s1',s2') \<longrightarrow> (\<exists> p1 p2 . path M q1 p1 \<and> path M q2 p2 \<and> p_io p1 = p_io p2 \<and> p_io p1 = p_io p \<and> target p1 q1 = s1' \<and> target p2 q2 = s2')"
  ...
  next
    ...
    have "map t_target p = map (Inl o t_target) p'"
      using * by auto
    then have "target (initial ?C) p = Inl (target p' (q1,q2))"
  ...
  then show  "\<And> s1' s2' . target (initial (canonical_separator M q1 q2)) p = Inl (s1',s2') \<Longrightarrow> (\<exists> p1 p2 . path M q1 p1 \<and> path M q2 p2 \<and> p_io p1 = p_io p2 \<and> p_io p1 = p_io p \<and> target p1 q1 = s1' \<and> target p2 q2 = s2')"
       and   "target (initial (canonical_separator M q1 q2)) p = Inr q1 \<Longrightarrow> (\<exists> p1 p2 t . path M q1 (p1@[t]) \<and> path M q2 p2 \<and> p_io (p1@[t]) = p_io p \<and> p_io p2 = butlast (p_io p)) \<and> (\<not>(\<exists> p2 . path M q2 p2 \<and> p_io p2 = p_io p))"
       and   "target (initial (canonical_separator M q1 q2)) p = Inr q2 \<Longrightarrow> (\<exists> p1 p2 t . path M q1 p1 \<and> path M q2 (p2@[t]) \<and> p_io p1 = butlast (p_io p) \<and> p_io (p2@[t]) = p_io p) \<and> (\<not>(\<exists> p1 . path M q1 p1 \<and> p_io p1 = p_io p))"
    by blast+
  ...
  next
    ...
    then have "t \<in> (transitions ?C)"
      by auto
    obtain s1 s2 where "t_source t = Inl (s1,s2)"
      using canonical_separator_t_source_isl[OF \<open>t \<in> (transitions ?C)\<close>]
...
end (* lemma canonical_separator_path_initial_ex :
  assumes "path (canonical_separator M q1 q2) (initial (canonical_separator M q1 q2)) p" (is "path ?C (initial ?C) p")
  and     "q1 \<in> nodes M"
  and     "q2 \<in> nodes M"
...
end (* lemma canonical_separator_language :
...
end (* lemma canonical_separator_language_prefix :
...
end (* lemma canonical_separator_distinguishing_transitions_left_h :
...
proof -
  ...
  then have x2: "t_input t \<in> (inputs ?C)" and x3: "t_output t \<in> (outputs ?C)"
...
end (* lemma canonical_separator_distinguishing_transitions_right_h :
...
end (* lemma canonical_separator_io_from_prefix_left :
...
proof -
  ...
  show ?thesis proof (cases "\<exists> tR \<in> (transitions M) . t_source tR = target pR q2 \<and> t_input tR = t_input tL \<and> t_output tR = t_output tL")
    case True
    then obtain tR where "tR \<in> (transitions M)" and "t_source tR = target pR q2" and "t_input tR = t_input tL" and "t_output tR = t_output tL"
      by blast
  ...
  next
    ...
    have f2: "(\<lambda> qqt. t_source (snd qqt) = fst (fst qqt) \<and> \<not> (\<exists>t'\<in>(transitions M). t_source t' = snd (fst qqt) \<and> t_input t' = t_input (snd qqt) \<and> t_output t' = t_output (snd qqt))) ((s1,s2),tL)"
    proof 
      show "t_source (snd ((s1, s2), tL)) = fst (fst ((s1, s2), tL))"
        using \<open>t_source tL = s1\<close> by auto 
      show "\<not> (\<exists>t'\<in>(transitions M). t_source t' = snd (fst ((s1, s2), tL)) \<and> t_input t' = t_input (snd ((s1, s2), tL)) \<and> t_output t' = t_output (snd ((s1, s2), tL)))"
        using False unfolding fst_conv snd_conv \<open>target pR q2 = s2\<close> by assumption
    qed
    
    have m1: "((s1,s2),tL) \<in> set (filter (\<lambda> qqt. t_source (snd qqt) = fst (fst qqt) \<and> \<not> (\<exists>t'\<in>(transitions M). t_source t' = snd (fst qqt) \<and> t_input t' = t_input (snd qqt) \<and> t_output t' = t_output (snd qqt)))
                                                (concat (map (\<lambda>qq'. map (Pair qq') (wf_transitions M)) (nodes_from_distinct_paths (product (from_FSM M q1) (from_FSM M q2))))))"
      using filter_list_set[OF f1, of "(\<lambda> qqt. t_source (snd qqt) = fst (fst qqt) \<and> \<not> (\<exists>t'\<in>(transitions M). t_source t' = snd (fst qqt) \<and> t_input t' = t_input (snd qqt) \<and> t_output t' = t_output (snd qqt)))", OF f2]
      by assumption
...
end (* lemma state_separator_language_intersections_nonempty :
  assumes "is_state_separator_from_canonical_separator (canonical_separator M q1 q2) q1 q2 A"
  and     "observable M"
  and     "q1 \<in> nodes M"
  and     "q2 \<in> nodes M"
...
end (* lemma state_separator_language_inclusion :
  assumes "is_state_separator_from_canonical_separator (canonical_separator M q1 q2) q1 q2 A"
  and     "q1 \<in> nodes M"
  and     "q2 \<in> nodes M"
...
end (* lemma state_separator_from_canonical_separator_targets_left_inclusion :
...
end (* lemma state_separator_from_canonical_separator_targets_right_inclusion :
  assumes "observable T" 
  and     "observable M"
  and     "t1 \<in> nodes T"
  and     "q1 \<in> nodes M"
...
end (* lemma trim_transitions_state_separator_from_canonical_separator : 
  assumes "is_state_separator_from_canonical_separator CSep q1 q2 S"
...
end (* lemma transition_reorder_is_state_separator_from_canonical_separator : 
  assumes "is_state_separator_from_canonical_separator CSep q1 q2 S"
  and     "initial S' = initial S"
  and     "inputs S' = inputs S"
  and     "outputs S' = outputs S"
  and     "(transitions S') = (transitions S)"
...
proof -
  have "is_submachine S CSep"
        ...
        and "(\<forall>q\<in>nodes S.
              \<forall>x\<in>(inputs CSep).
                 ...
                 (\<forall>t'\<in> transitions CSep. t_source t' = q \<and> t_input t' = x \<longrightarrow> t' \<in> transitions S))"
  ...
  moreover have "(\<forall>q\<in>nodes S'.
    ...
    using \<open>(\<forall>q\<in>nodes S.
              \<forall>x\<in>(inputs CSep).
                 (\<exists>t\<in> transitions S. t_source t = q \<and> t_input t = x) \<longrightarrow>
                 (\<forall>t'\<in> transitions CSep. t_source t' = q \<and> t_input t' = x \<longrightarrow> t' \<in> transitions S))\<close> 
...
end (* lemma calculate_state_separator_from_canonical_separator_naive_soundness :
...
shows "is_state_separator_from_canonical_separator (canonical_separator M q1 q2) q1 q2 S"
  ...
  using find_condition[of "(is_state_separator_from_canonical_separator (canonical_separator M q1 q2) q1 q2)"
                          "(map (generate_submachine_from_assignment (canonical_separator M q1 q2))
                             (generate_choices
                               (map (\<lambda>q. (q, filter (\<lambda>x. \<exists>t\<in> transitions (canonical_separator M q1 q2). t_source t = q \<and> t_input t = x) (inputs (canonical_separator M q1 q2))))
                                 (nodes_from_distinct_paths (canonical_separator M q1 q2)))))", of S] 
  by metis 


end (* lemma calculate_state_separator_from_canonical_separator_naive_exhaustiveness :
...
proof -
  ...
  moreover have "\<And> NS . finite NS \<Longrightarrow> NS \<subseteq> nodes ?CSep 
        \<Longrightarrow> \<exists> f . (\<forall> q . (q \<notin> NS \<longrightarrow> f q = None) \<and> (q \<in> NS \<longrightarrow> ((q \<notin> nodes ?S \<and> f q = None) \<or> (q \<in> nodes ?S \<and> deadlock_state ?S q \<and> f q = None) \<or> (q \<in> nodes ?S \<and> \<not> deadlock_state ?S q \<and> f q = Some (THE x .  \<exists>t\<in> transitions ?S. t_source t = q \<and> t_input t = x)))))"
  proof -
    fix NS assume "finite NS" and "NS \<subseteq> nodes ?CSep"
    then show "\<exists> f . (\<forall> q . (q \<notin> NS \<longrightarrow> f q = None) \<and> (q \<in> NS \<longrightarrow> ((q \<notin> nodes ?S \<and> f q = None) \<or> (q \<in> nodes ?S \<and> deadlock_state ?S q \<and> f q = None) \<or> (q \<in> nodes ?S \<and> \<not> deadlock_state ?S q \<and> f q = Some (THE x .  \<exists>t\<in> transitions ?S. t_source t = q \<and> t_input t = x)))))"
    proof (induction)
      case empty
      then show ?case by auto
    next
      ...
      then obtain f where f_def : "(\<forall> q . (q \<notin> NS \<longrightarrow> f q = None) \<and> (q \<in> NS \<longrightarrow> ((q \<notin> nodes ?S \<and> f q = None) \<or> (q \<in> nodes ?S \<and> deadlock_state ?S q \<and> f q = None) \<or> (q \<in> nodes ?S \<and> \<not> deadlock_state ?S q \<and> f q = Some (THE x .  \<exists>t\<in> transitions ?S. t_source t = q \<and> t_input t = x)))))"
      ...
      show ?case proof (cases "s \<in> nodes ?S")
        ...
        then show ?thesis proof (cases "deadlock_state ?S s")
          case True
          let ?f = "f( s := None)"
          have "(\<forall> q . (q \<notin> (insert s NS) \<longrightarrow> f q = None) \<and> (q \<in> (insert s NS) \<longrightarrow> ((q \<notin> nodes ?S \<and> f q = None) \<or> (q \<in> nodes ?S \<and> deadlock_state ?S q \<and> f q = None) \<or> (q \<in> nodes ?S \<and> \<not> deadlock_state ?S q \<and> f q = Some (THE x .  \<exists>t\<in> transitions ?S. t_source t = q \<and> t_input t = x)))))"
            by (metis (no_types, lifting) True f_def insert_iff)
          then show ?thesis by blast
        next
          case False
          then obtain t where "t\<in> transitions ?S" and "t_source t = s"
            by (meson True deadlock_state.elims(3))
          then have theEx: "\<exists>t'\<in> transitions ?S. t_source t' = s \<and> t_input t' = t_input t" 
            using \<open>t_source t = s\<close> \<open>s \<in> nodes ?S\<close> by blast
          
          have "\<forall> t' \<in> transitions ?S . (t_source t' = s) \<longrightarrow> t_input t' = t_input t"
            using \<open>single_input ?S\<close> \<open>t_source t = s\<close> \<open>s \<in> nodes ?S\<close> unfolding single_input.simps
            by (metis \<open>t \<in> transitions ?S\<close>) 
          then have theAll: "\<And>x . (\<exists>t'\<in> transitions ?S. t_source t' = s \<and> t_input t' = x) \<Longrightarrow> x = t_input t"
            by blast


          let ?f = "f( s := Some (t_input t))"
          have "t_input t = (THE x .  \<exists>t'\<in> transitions ?S. t_source t' = s \<and> t_input t' = x)"
            using the_equality[of "\<lambda> x . \<exists>t'\<in> transitions ?S. t_source t' = s \<and> t_input t' = x" "t_input t", OF theEx theAll] by simp


          then have "(\<forall> q . (q \<notin> (insert s NS) \<longrightarrow> ?f q = None) \<and> (q \<in> (insert s NS) \<longrightarrow> ((q \<notin> nodes ?S \<and> ?f q = None) \<or> (q \<in> nodes ?S \<and> deadlock_state ?S q \<and> ?f q = None) \<or> (q \<in> nodes ?S \<and> \<not> deadlock_state ?S q \<and> ?f q = Some (THE x .  \<exists>t\<in> transitions ?S. t_source t = q \<and> t_input t = x)))))"
  ...
  ultimately obtain f where f_def : "(\<forall> q . 
                                        ...
                                        \<and> (q \<in> nodes ?S \<longrightarrow> ((q \<notin> nodes ?S \<and> f q = None) 
                                                            ...
                                                            \<or> (q \<in> nodes ?S \<and> \<not> deadlock_state ?S q \<and> f q = Some (THE x .  \<exists>t\<in> transitions ?S. t_source t = q \<and> t_input t = x)))))"
    by blast
  ...
  proof -
    ...
    have "snd (?possible_choices ! i) = filter (\<lambda>x . \<exists> t \<in> transitions ?CSep . t_source t = ?q \<and> t_input t = x) (inputs ?CSep)"
     ...
     proof (cases "?q \<in> nodes ?S")
      ...
      next
        case False
        then obtain x where "\<exists>t\<in> transitions ?S. t_source t = ?q \<and> t_input t = x"
          by (metis (no_types, lifting) deadlock_state.elims(3))
        then have "\<exists>! x . \<exists>t\<in> transitions ?S. t_source t = ?q \<and> t_input t = x"
          using \<open>single_input ?S\<close> unfolding single_input.simps
          by (metis (mono_tags, lifting)) 
        then have "(THE x .  \<exists>t\<in> transitions ?S. t_source t = ?q \<and> t_input t = x) = x"
          using the1_equality[of "\<lambda> x . \<exists>t\<in> transitions ?S. t_source t = ?q \<and> t_input t = x"] \<open>\<exists>t\<in> transitions ?S. t_source t = ?q \<and> t_input t = x\<close> by blast
        moreover have "f ?q = Some (THE x .  \<exists>t\<in> transitions ?S. t_source t = ?q \<and> t_input t = x)"
        ...
        then have "\<exists> t \<in> transitions ?CSep . t_source t = ?q \<and> t_input t = x"
          using \<open>\<exists>t\<in> transitions ?S. t_source t = ?q \<and> t_input t = x\<close> by blast
        moreover have "x \<in> (inputs ?CSep)"
          using \<open>\<exists>t\<in> transitions ?S. t_source t = ?q \<and> t_input t = x\<close> \<open>is_submachine ?S ?CSep\<close> by auto
        ultimately have **: "x \<in> set (filter (\<lambda>x . \<exists> t \<in> transitions ?CSep . t_source t = ?q \<and> t_input t = x) (inputs ?CSep))"
          by auto

        have "the (snd (?assn ! i)) \<in> set (snd (?possible_choices ! i))"
          using * ** \<open>snd (?possible_choices ! i) = filter (\<lambda>x . \<exists> t \<in> transitions ?CSep . t_source t = ?q \<and> t_input t = x) (inputs ?CSep)\<close> \<open>snd (?assn ! i) = f ?q\<close> by fastforce 
  ...
  have "(transitions ?S) = set ?filtered_transitions"
  proof -
    have "\<And> t . t \<in> (transitions ?S) \<Longrightarrow> t \<in> set (?filtered_transitions)"
    proof -
      fix t assume "t \<in> (transitions ?S)"
      then have "t \<in> (transitions ?CSep)"
      ...
      have "t \<in> transitions ?S"
        using trim_transitions_transitions \<open>t \<in> (transitions ?S)\<close> by auto

      have "t_source t \<in> nodes ?S"
        using trim_transitions_t_source[OF \<open>t \<in> (transitions ?S)\<close>] by assumption
      have "\<not> deadlock_state ?S (t_source t)"
        unfolding deadlock_state.simps using \<open>t \<in> transitions ?S\<close> by blast
      
      have the1: "\<exists>t'\<in> transitions ?S. t_source t' = t_source t \<and> t_input t' = t_input t"
        using \<open>t \<in> transitions ?S\<close> by blast
      then have the2: "\<exists>! x . \<exists>t'\<in> transitions ?S. t_source t' = t_source t \<and> t_input t' = x"
      ...
      then show "t \<in> set (?filtered_transitions)"
        using filter_list_set[OF \<open>t \<in> (transitions ?CSep)\<close>, of "(\<lambda> t . (t_source t, Some (t_input t)) \<in> set (map (\<lambda>q. (q, f q)) (nodes_from_distinct_paths ?CSep)))"] by blast
    qed

    moreover have "\<And> t . t \<in> set (?filtered_transitions) \<Longrightarrow> t \<in> (transitions ?S)"
    proof -
      ...
      qed
      ultimately have "f (t_source t) = Some (THE x. \<exists>t'\<in> transitions ?S. t_source t' = t_source t \<and> t_input t' = x)"
        using f_def by auto
      then have "t_input t = (THE x. \<exists>t'\<in> transitions ?S. t_source t' = t_source t \<and> t_input t' = x)"
        using \<open>f (t_source t) = Some (t_input t)\<close> by auto


      have "\<exists> x . \<exists>t'\<in> transitions ?S. t_source t' = t_source t \<and> t_input t' = x"
      ...
      then obtain x where the1: \<open>\<exists>t'\<in> transitions ?S. t_source t' = t_source t \<and> t_input t' = x\<close> by blast
      then have the2: "\<exists>! x . \<exists>t'\<in> transitions ?S. t_source t' = t_source t \<and> t_input t' = x"
        using \<open>single_input ?S\<close> unfolding single_input.simps by metis

      have "x = t_input t"
        using \<open>t_input t = (THE x. \<exists>t'\<in> transitions ?S. t_source t' = t_source t \<and> t_input t' = x)\<close> the1_equality[OF the2 the1] by auto
      then have "(\<exists>t'\<in> transitions ?S. t_source t' = t_source t \<and> t_input t' = t_input t)"
        using the1 by blast

      have "t_input t \<in> (inputs ?CSep)"
        using \<open>t \<in> (transitions ?CSep)\<close> by auto
      
      have "(\<forall>t'\<in> transitions ?CSep. t_source t' = t_source t \<and> t_input t' = t_input t \<longrightarrow> t' \<in> transitions ?S)"
        ...
        using \<open>t_source t \<in> nodes ?S\<close>
              \<open>t_input t \<in> (inputs ?CSep)\<close>
              \<open>\<exists>t'\<in> transitions ?S. t_source t' = t_source t \<and> t_input t' = t_input t\<close> by blast
      then have "t \<in> transitions ?S"
        using \<open>t \<in> (transitions ?CSep)\<close> by blast
      then show "t \<in> (transitions ?S)"
  ...
  moreover have "(transitions ?S') = set (?filtered_transitions)" 
    unfolding generate_submachine_from_assignment.simps by fastforce
  ultimately have "(transitions ?S') = (transitions ?S)"
    by blast
  ...
  then have "is_state_separator_from_canonical_separator ?CSep q1 q2 ?S'"
    using transition_reorder_is_state_separator_from_canonical_separator[OF \<open>is_state_separator_from_canonical_separator ?CSep q1 q2 ?S\<close> _ _ _ \<open>(transitions ?S') = (transitions ?S)\<close>] by metis
...
end (* lemma calculate_state_separator_from_canonical_separator_naive_ex :
...
definition induces_state_separator :: "('a,'b,'c) fsm \<Rightarrow> ('a \<times> 'a) FSM \<Rightarrow> bool" where
...
definition induces_state_separator_for_prod :: "('a,'b,'c) fsm \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> ('a \<times> 'a) FSM \<Rightarrow> bool" where
...
proof -
  ...
  proof -
    ...
    next
      ...
      have *: "(transitions ?CSep) = (set (shifted_transitions M ?q1 ?q2)) \<union> (set (distinguishing_transitions_left M ?q1 ?q2)) \<union> (set (distinguishing_transitions_right M ?q1 ?q2))"
        using canonical_separator_simps(4)[of M ?q1 ?q2] by auto
...
fun s_states :: "('a,'b,'c) fsm \<Rightarrow> nat \<Rightarrow> ('a \<times> Input) list" where
  "s_states C 0 = []" |
  "s_states C (Suc k) =  
...
fun s_states_opt :: "('a,'b,'c) fsm \<Rightarrow> nat \<Rightarrow> ('a \<times> Input) list" where
  ...
  proof -
    ...
    proof -
      ...
      next
        ...
        then have "(Inl (target p1 q1, target p2 q2), x, yq, Inr q1) \<in> transitions ?C"
          using canonical_separator_distinguishing_transitions_left_h by metis
