...
proof -
  ...
  have "(q1',q2') \<in> reachable_nodes ?P"
  ...
  proof (rule ccontr)
    ...
    then have "(Inl (q1',q2'),x,y,Inl (q1'',q2'')) \<in> transitions ?C"
      using \<open>(q1',q2') \<in> reachable_nodes ?P\<close> unfolding canonical_separator_transitions_def[OF assms(5,6)] h_out.simps by blast
    then show "False"
      using assms(4) by auto
  qed
  ...
  have "?t1 \<in> distinguishing_transitions_left M q1 q2"
    using \<open>(q1',q2') \<in> reachable_nodes ?P\<close> \<open>y1 \<in> h_out M (q1',x) - h_out M (q2',x)\<close>
  ...
  have "?t2 \<in> distinguishing_transitions_right M q1 q2"
    using \<open>(q1',q2') \<in> reachable_nodes ?P\<close> \<open>y2 \<in> h_out M (q2',x) - h_out M (q1',x)\<close>
...
proof -
  ...
  *)
  ...
  have "(Inl (q1',q2'), x, y1, Inr q1) \<in> transitions ?S"
    using \<open>((q1',q2'),x) \<in> set cs\<close> from_input_choices_transitions_alt_def[OF assms(2,3,4,5,6)] \<open>(Inl (q1',q2'),x,y1,Inr q1) \<in> transitions (canonical_separator M q1 q2)\<close> by force

  have "(Inl (q1',q2'), x, y2, Inr q2) \<in> transitions ?S"
    using \<open>((q1',q2'),x) \<in> set cs\<close> from_input_choices_transitions_alt_def[OF assms(2,3,4,5,6)] \<open>(Inl (q1',q2'),x,y2,Inr q2) \<in> transitions (canonical_separator M q1 q2)\<close> by force
...
value "(\<lambda> (M::(integer, integer, integer) fsm) q1 q2 k . let P = restrict_to_reachable_nodes (product (from_FSM M q1) (from_FSM M q2)); 
                        f = h P;
                        q = (q1,q2);
                        q0 = (q1,q2);
                        inputList = inputs_as_list P;
                        nodeList  = removeAll q0 (nodes_as_list P);
                        nodeSet   = {};
                        m         = []
                      in d_states' f q q0 inputList nodeList nodeSet k m) m_ex_H 1 3 1"

value "(\<lambda> (M::(integer, integer, integer) fsm) q1 q2 k . let C = canonical_separator M q1 q2; 
                        f = h C;
                        q = Inl (q1,q2);
                        q0 = Inl (q1,q2);
                        inputList = inputs_as_list C;
                        nodeList  = removeAll q0 (nodes_as_list C);
                        nodeSet   = {Inr q1, Inr q2};
                        m         = []
                      in d_states' f q q0 inputList nodeList nodeSet k m) m_ex_H 1 3 2"



fun s_states' :: "((('a \<times> 'a) \<times> 'b) \<Rightarrow> ('c \<times> ('a \<times> 'a)) set) \<Rightarrow> ('a \<times> 'a) \<Rightarrow> ('a \<times> 'a) \<Rightarrow> 'b list \<Rightarrow> ('a \<times> 'a) list \<Rightarrow> ('a \<times> 'a) set \<Rightarrow> nat \<Rightarrow> (('a \<times> 'a) \<times> 'b) list \<Rightarrow> (('a \<times> 'a) \<times> 'b) list" where
  "s_states' f q q0 inputList nodeList nodeSet 0 m = (case find (\<lambda> x . f (q0,x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q0,x) . (q'' \<in> nodeSet))) inputList of 
      Some x \<Rightarrow> m@[(q0,x)] |
      None   \<Rightarrow> m)" |
  "s_states' f q q0 inputList nodeList nodeSet (Suc k) m = 
    (case find (\<lambda> x . f (q0,x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q0,x) . (q'' \<in> nodeSet))) inputList of 
      ...
      None   \<Rightarrow> (case find_remove_2 (\<lambda> q' x . f (q',x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q',x) . (q'' \<in> nodeSet))) nodeList inputList
          of None            \<Rightarrow> m |
             Some (q',x,nodeList') \<Rightarrow> s_states' f q q0 inputList nodeList' (insert q' nodeSet) k (m@[(q',x)])))"
