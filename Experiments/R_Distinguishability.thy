...
proof -
  ...
  show ?thesis
    using r_0_distinguishable_from_not_completely_specified_initial[OF _ \<open>q1' \<in> nodes M\<close> \<open>q2' \<in> nodes M\<close>]
          assms(1)
...
next
  ...
  next
    ...
    have "fst (t_target t) \<in> FSM.nodes M" and "snd (t_target t) \<in> FSM.nodes M"
      using fsm_transition_target[OF \<open>t \<in> transitions ?P\<close>] unfolding product_simps from_FSM_simps[OF \<open>q1 \<in> nodes M\<close>] from_FSM_simps[OF \<open>q2 \<in> nodes M\<close>] by auto
