...
fun select_inputs :: "(('a \<times> 'b) \<Rightarrow> ('c \<times> 'a) set) \<Rightarrow> 'a \<Rightarrow> 'b list \<Rightarrow> 'a list \<Rightarrow> 'a set \<Rightarrow> nat \<Rightarrow> ('a \<times> 'b) list \<Rightarrow> ('a \<times> 'b) list" where
  "select_inputs f q0 inputList nodeList nodeSet 0 m = (case find (\<lambda> x . f (q0,x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q0,x) . (q'' \<in> nodeSet))) inputList of 
      Some x \<Rightarrow> m@[(q0,x)] |
      None   \<Rightarrow> m)" |
  "select_inputs f q0 inputList nodeList nodeSet (Suc k) m = 
    (case find (\<lambda> x . f (q0,x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q0,x) . (q'' \<in> nodeSet))) inputList of 
      ...
      None   \<Rightarrow> (case find_remove_2 (\<lambda> q' x . f (q',x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q',x) . (q'' \<in> nodeSet))) nodeList inputList
          of None            \<Rightarrow> m |
             Some (q',x,nodeList') \<Rightarrow> select_inputs f q0 inputList nodeList' (insert q' nodeSet) k (m@[(q',x)])))"


lemma select_inputs_length :
  "length (select_inputs f q0 inputList nodeList nodeSet k m) \<le> (length m) + Suc k"
...
lemma select_inputs_length_min :
  "length (select_inputs f q0 inputList nodeList nodeSet k m) \<ge> (length m)"
proof (induction k arbitrary: nodeList nodeSet m)
  case 0
  then show ?case by (cases "find (\<lambda>x. f (q0, x) \<noteq> {} \<and> (\<forall>(y, q'')\<in>f (q0, x). q'' \<in> nodeSet)) inputList"; auto)
next
...
lemma select_inputs_helper1 :
  "find (\<lambda>x. f (q0, x) \<noteq> {} \<and> (\<forall>(y, q'')\<in>f (q0, x). q'' \<in> nS)) iL = Some x \<Longrightarrow> (select_inputs f q0 iL nL nS k m) = m@[(q0,x)]" 
  by (cases k; auto)
...
lemma select_inputs_take :
  "take (length m) (select_inputs f q0 iL nL nS k m) = m"
...
next
  ...
  then show ?case proof (cases "find (\<lambda> x . f (q0,x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q0,x) . (q'' \<in> nS))) iL")
    ...
    next
      ...
      have "take (length (m @ [(q', x)])) (select_inputs f q0 iL nL nS (Suc k) m) = (m @ [(q', x)])"
        unfolding select_inputs.simps None *
...
lemma select_inputs_take' :
  "take (length m) (select_inputs f q0 iL nL nS k (m@m')) = m"
...
lemma select_inputs_distinct :
  ...
  and     "q0 \<notin> set nL"
  and     "set nL \<inter> nS = {}"
  shows "distinct (map fst (select_inputs f q0 iL nL nS k m))" 
...
next
  ...
  then show ?case proof (cases "find (\<lambda> x . f (q0,x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q0,x) . (q'' \<in> nS))) iL")
    ...
    then show ?thesis proof (cases "find_remove_2 (\<lambda> q' x . f (q',x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q',x) . (q'' \<in> nS))) nL iL")
      case None
      then have "(select_inputs f q0 iL nL nS (Suc k) m) = m"
        using \<open>find (\<lambda>x. f (q0, x) \<noteq> {} \<and> (\<forall>(y, q'')\<in>f (q0, x). q'' \<in> nS)) iL = None\<close> by auto
...
lemma select_inputs_index_properties : 
  assumes "i < length (select_inputs (h M) q0 iL nL nS k m)"
  and     "i \<ge> length m"
  ...
  and     "q0 \<notin> set nL"
  and     "set nL \<inter> nS = {}"
shows "fst (select_inputs (h M) q0 iL nL nS k m ! i) \<in> (insert q0 (set nL))" 
      "fst (select_inputs (h M) q0 iL nL nS k m ! i) \<notin> nS0"
      "snd (select_inputs (h M) q0 iL nL nS k m ! i) \<in> set iL"
      "(\<forall> qx' \<in> set (take i (select_inputs (h M) q0 iL nL nS k m)) . fst (select_inputs (h M) q0 iL nL nS k m ! i) \<noteq> fst qx')"
...
proof -
  ...
  using assms proof (induction k arbitrary: nL nS m)
    ...
    show ?case proof (cases "find (\<lambda> x . (h M) (q0,x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> (h M) (q0,x) . (q'' \<in> nS))) iL")
      case None
      then have "(select_inputs (h M) q0 iL nL nS 0 m) = m" by auto
      then have "False" using "0.prems"(1,2) by auto
      then show ?thesis by simp
    next
      case (Some x)
      have "(select_inputs (h M) q0 iL nL nS 0 m) = m@[(q0,x)]" using select_inputs_helper1[OF Some] by assumption
      then have "(select_inputs (h M) q0 iL nL nS 0 m ! i) = (q0,x)" using "0.prems"(1,2)
      ...
      moreover have "\<And> t . t \<in> FSM.transitions M \<Longrightarrow>
          ...
          t_target t \<in> nS0 \<or> (\<exists>qx'\<in>set (take i (select_inputs (h M) q0 iL nL nS 0 m)). fst qx' = t_target t)"   
      proof -
        ...
        then show "t_target t \<in> nS0 \<or> (\<exists>qx'\<in>set (take i (select_inputs (h M) q0 iL nL nS 0 m)). fst qx' = t_target t)"
          using \<open>nS = nS0 \<union> set (map fst m)\<close>
          using "0.prems"(1) "0.prems"(2) \<open>select_inputs (h M) q0 iL nL nS 0 m = m @ [(q0, x)]\<close> by fastforce    
      qed
        
      ultimately show ?thesis 
        unfolding \<open>(select_inputs (h M) q0 iL nL nS 0 m ! i) = (q0,x)\<close> by blast
    qed
  next
    case (Suc k)
    ...
    show ?case proof (cases "find (\<lambda> x . (h M) (q0,x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> (h M) (q0,x) . (q'' \<in> nS))) iL")
      ...
      show ?thesis proof (cases "find_remove_2 (\<lambda> q' x . (h M) (q',x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> (h M) (q',x) . (q'' \<in> nS))) nL iL")
        case None
        then have "(select_inputs (h M) q0 iL nL nS (Suc k) m) = m" using \<open>find (\<lambda>x. h M (q0, x) \<noteq> {} \<and> (\<forall>(y, q'')\<in>h M (q0, x). q'' \<in> nS)) iL = None\<close> by auto
      ...
      next
        ...
        then obtain q' x nL' where **: "find_remove_2 (\<lambda> q' x . (h M) (q',x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> (h M) (q',x) . (q'' \<in> nS))) nL iL = Some (q',x,nL')"
          by (metis prod_cases3)
        then have "select_inputs (h M) q0 iL nL nS (Suc k) m = select_inputs (h M) q0 iL nL' (insert q' nS) k (m@[(q',x)])"
          using None by auto
        then have "i < length (select_inputs (h M) q0 iL nL' (insert q' nS) k (m@[(q',x)]))"
        ...
        show ?thesis proof (cases "length (m @ [(q', x)]) \<le> i")
          ...
          show ?thesis
            ...
            using Suc.IH[OF \<open>i < length (select_inputs (h M) q0 iL nL' (insert q' nS) k (m@[(q',x)]))\<close>
                            True
            ...
            unfolding \<open>select_inputs (h M) q0 iL nL nS (Suc k) m = select_inputs (h M) q0 iL nL' (insert q' nS) k (m@[(q',x)])\<close> 
            using \<open>set nL' \<subseteq> set nL\<close> by blast
        next
          case False 
          then have "i = length m"
            using Suc.prems(2) by auto
          then have ***: "select_inputs (h M) q0 iL nL nS (Suc k) m ! i = (q',x)"
            unfolding \<open>select_inputs (h M) q0 iL nL nS (Suc k) m = select_inputs (h M) q0 iL nL' (insert q' nS) k (m@[(q',x)])\<close> 
            using select_inputs_take
    ...
    next
      case (Some x)
      have "(select_inputs (h M) q0 iL nL nS (Suc k) m) = m@[(q0,x)]" using select_inputs_helper1[OF Some] by assumption
      then have "(select_inputs (h M) q0 iL nL nS (Suc k) m ! i) = (q0,x)" using Suc.prems(1,2)
        using antisym by fastforce 
      ...
      moreover have "\<And> t . t \<in> FSM.transitions M \<Longrightarrow>
          t_source t = fst (q0, x) \<Longrightarrow> t_input t = snd (q0, x) \<Longrightarrow>
          t_target t \<in> nS0 \<or> (\<exists>qx'\<in>set (take i (select_inputs (h M) q0 iL nL nS (Suc k) m)). fst qx' = t_target t)"   
      proof -
        fix t assume "t \<in> FSM.transitions M" "t_source t = fst (q0, x)" "t_input t = snd (q0, x)"
        then have "t_target t \<in> nS"
          using find_condition[OF Some] unfolding h.simps fst_conv snd_conv
          by (metis (mono_tags, lifting) case_prod_beta' mem_Collect_eq prod.collapse) 
        then show "t_target t \<in> nS0 \<or> (\<exists>qx'\<in>set (take i (select_inputs (h M) q0 iL nL nS (Suc k) m)). fst qx' = t_target t)"
          using \<open>nS = nS0 \<union> (set (map fst m))\<close>
          using Suc.prems(1,2) \<open>select_inputs (h M) q0 iL nL nS (Suc k) m = m @ [(q0, x)]\<close> by fastforce    
      qed
        
      ultimately show ?thesis 
        unfolding \<open>(select_inputs (h M) q0 iL nL nS (Suc k) m ! i) = (q0,x)\<close> by blast
    qed
  qed
  ...
  show "(\<forall> qx' \<in> set (take i (select_inputs (h M) q0 iL nL nS k m)) . fst (select_inputs (h M) q0 iL nL nS k m ! i) \<noteq> fst qx')" 
  proof 
    fix qx' assume "qx' \<in> set (take i (select_inputs (h M) q0 iL nL nS k m))"
    then obtain j where "(take i (select_inputs (h M) q0 iL nL nS k m)) ! j = qx'" and "j < length (take i (select_inputs (h M) q0 iL nL nS k m))"
      by (meson in_set_conv_nth)
    then have "fst qx' = (map fst (select_inputs (h M) q0 iL nL nS k m)) ! j" and "j < length (select_inputs (h M) q0 iL nL nS k m)" by auto
      
    moreover have "fst (select_inputs (h M) q0 iL nL nS k m ! i) = (map fst (select_inputs (h M) q0 iL nL nS k m)) ! i"
      using assms(1) by auto

    moreover have "j \<noteq> i" 
      using \<open>j < length (take i (select_inputs (h M) q0 iL nL nS k m))\<close> by auto

    moreover have "set (map fst m) \<subseteq> nS"
      using \<open>nS = nS0 \<union> set (map fst m)\<close> by blast

    ultimately show "fst (select_inputs (h M) q0 iL nL nS k m ! i) \<noteq> fst qx'"
      using assms(1)
...
lemma select_inputs_initial :
  assumes "qx \<in> set (select_inputs f q0 iL nL nS k m) - set m"
  and     "fst qx = q0"
  shows "(last (select_inputs f q0 iL nL nS k m)) = qx"
using assms(1) proof (induction k arbitrary: nS nL m)
  ...
  have "set (select_inputs f q0 iL nL nS k m) - set m = {qx}"
...
next
  ...
  show ?case proof (cases "find (\<lambda>x. f (q0, x) \<noteq> {} \<and> (\<forall>(y, q'')\<in>f (q0, x). q'' \<in> nS)) iL")
    ...
    show ?thesis proof (cases "find_remove_2 (\<lambda> q' x . f (q',x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q',x) . (q'' \<in> nS))) nL iL")
      case None
      have "(select_inputs f q0 iL nL nS (Suc k) m) = m"
    ...
    next
      ...
      then obtain q' x nL' where **: "find_remove_2 (\<lambda> q' x . f (q',x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q',x) . (q'' \<in> nS))) nL iL = Some (q',x,nL')"
        by (metis prod_cases3)
      then have "(select_inputs f q0 iL nL nS (Suc k) m) = (select_inputs f q0 iL nL' (insert q' nS) k (m@[(q',x)]))"
        using None by auto
      then have "qx \<in> set (select_inputs f q0 iL nL' (insert q' nS) k (m@[(q',x)])) - set m"
        using Suc.prems by auto
      moreover have "q0 \<noteq> q'"
        using None unfolding find_None_iff
        using find_remove_2_set(1,2,3)[OF **]
        by blast
      ultimately have "qx \<in> set (select_inputs f q0 iL nL' (insert q' nS) k (m@[(q',x)])) - set (m@[(q',x)])"
        using \<open>fst qx = q0\<close> by auto
      then show ?thesis 
        using Suc.IH unfolding \<open>(select_inputs f q0 iL nL nS (Suc k) m) = (select_inputs f q0 iL nL' (insert q' nS) k (m@[(q',x)]))\<close> by metis
    qed
  next
    case (Some a)
...
lemma select_inputs_max_length :
  assumes "distinct nL"
  shows "length (select_inputs f q0 iL nL nS k m) \<le> length m + Suc (length nL)" 
using assms proof (induction k arbitrary: nL nS m)
  case 0
  then show ?case by (cases "find (\<lambda> x . f (q0,x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q0,x) . (q'' \<in> nS))) iL"; auto)
next
  ...
  show ?case proof (cases "find (\<lambda> x . f (q0,x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q0,x) . (q'' \<in> nS))) iL")
    ...
    show ?thesis proof (cases "find_remove_2 (\<lambda> q' x . f (q',x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q',x) . (q'' \<in> nS))) nL iL")
      case None
      show ?thesis unfolding select_inputs.simps None \<open>find (\<lambda>x. f (q0, x) \<noteq> {} \<and> (\<forall>(y, q'')\<in>f (q0, x). q'' \<in> nS)) iL = None\<close> by auto
    next
      case (Some a)
      then obtain q' x nL' where **: "find_remove_2 (\<lambda> q' x . f (q',x) \<noteq> {} \<and> (\<forall> (y,q'') \<in> f (q',x) . (q'' \<in> nS))) nL iL = Some (q',x,nL')"
        by (metis prod_cases3)
      ...
      then have "length (select_inputs f q0 iL nL' (insert q' nS) k (m@[(q',x)])) \<le> length m + Suc (length nL)"
        using Suc.IH
        by (metis add_Suc_shift length_append_singleton)
      then show ?thesis 
        using \<open>(select_inputs f q0 iL nL nS (Suc k) m) = select_inputs f q0 iL nL' (insert q' nS) k (m@[(q',x)])\<close> by simp
    qed
  next
    case (Some a)
...
lemma select_inputs_q0_containment :
  ...
  and     "x \<in> set iL"
shows "(\<exists> qx \<in> set (select_inputs f q0 iL nL nS k m) . fst qx = q0)" 
proof -
