...
lemma is_ATC_from :
  ...
  and     "t_source t \<in> reachable_nodes A"
  and     "is_ATC A"
shows "is_ATC (from_FSM A (t_target t))"
  using from_FSM_single_input[of A]
        from_FSM_acyclic[OF reachable_nodes_next[OF assms(2,1)]] 
...
fun pass_ATC' :: "('a,'b,'c) fsm \<Rightarrow> ('d,'b,'c) fsm \<Rightarrow> 'd set \<Rightarrow> nat \<Rightarrow> bool" where
  ...
  "pass_ATC' M A fail_states (Suc k) = ((\<not> (initial A \<in> fail_states)) \<and> 
    (\<forall> x \<in> inputs A . \<forall> (yA,qA) \<in> h A (initial A,x) . \<forall> (yM,qM) \<in> h M (initial M,x) . yM \<in> h_out A (initial A,x) \<and> pass_ATC' (from_FSM M qM) (from_FSM A qA) fail_states k))"
  ...
  next
    ...
    then show ?case proof (cases io)
      ...
      have "find (\<lambda> x . \<exists> t \<in> transitions A . t_input t = x \<and> t_source t = initial A) (inputs A) = Some (fst ioA)"
        ...
        using * \<open>tA \<in> transitions A\<close> \<open>t_source tA = initial A\<close> by blast

      
      then have ***: "\<And> tM . tM \<in> transitions M \<Longrightarrow> t_input tM = fst ioA \<Longrightarrow> t_source tM = initial M \<Longrightarrow>
        (\<exists> tA \<in> transitions A .
    ...
    next
      ...
      have *: "\<And> x . x \<in> (inputs A) \<Longrightarrow> \<exists> t' \<in> transitions A . t_input t' = x \<and> t_source t' = initial A \<Longrightarrow> x = fst io'"
        using \<open>is_ATC A\<close> \<open>tA \<in> transitions A\<close> unfolding is_ATC_def single_input.simps
      ...
      have "find (\<lambda> x . \<exists> t \<in> transitions A . t_input t = x \<and> t_source t = initial A) (inputs A) = Some (fst io')"
        using find_scheme[OF \<open>fst io' \<in> (inputs A)\<close>, of "\<lambda>x . \<exists> t' \<in> transitions A . t_input t' = x \<and> t_source t' = initial A"]
        using * \<open>tA \<in> transitions A\<close> \<open>t_source tA = initial A\<close> by blast

      
      then have ***: "\<And> tM . tM \<in> transitions M \<Longrightarrow> t_input tM = fst io' \<Longrightarrow> t_source tM = initial M \<Longrightarrow>
        (\<exists> tA \<in> transitions A .
      ...
      have "observable (from_FSM M (t_target tM))"
        using \<open>observable M\<close> \<open>tM \<in> transitions M\<close>
      ...
      have "io'' @ [ioM] \<in> LS (from_FSM A (t_target tA)) (initial (from_FSM A (t_target tA)))"
       and "io_targets (from_FSM A (t_target tA)) (io'' @ [ioM]) (initial (from_FSM A (t_target tA))) \<inter> FS = {}" 
        using Suc.IH[OF \<open>pass_ATC' (from_FSM M (t_target tM)) (from_FSM A (t_target tA)) FS k\<close>
                        is_ATC_from[OF \<open>tA \<in> transitions A\<close> \<open>is_ATC A\<close>]
      ...
      have "set (filter (\<lambda>t. t_source t = initial A \<and> t_input t = fst io' \<and> t_output t = snd io') (wf_transitions A)) = {tA}"
        using ex_scheme[of "wf_transitions A" "(\<lambda> t' . t_source t' = initial A \<and> t_input t' = fst io' \<and> t_output t' = snd io')", OF
                          observable_transition_unique[OF \<open>observable A\<close> \<open>tA \<in> transitions A\<close> \<open>t_source tA = initial A\<close> \<open>t_input tA = fst io'\<close> \<open>t_output tA = snd io'\<close>]]
        using \<open>tA \<in> transitions A\<close> \<open>t_source tA = initial A\<close> \<open>t_input tA = fst io'\<close> \<open>t_output tA = snd io'\<close>
        by blast
      ...
      then have "io_targets A (io' # (io'' @ [ioM])) (initial A) = io_targets A (io'' @ [ioM]) (t_target tA)"
        using nodes.initial[of A] wf_transition_target[OF \<open>tA \<in> transitions A\<close>]
      ...
      then have "io_targets A (io' # (io'' @ [ioM])) (initial A) = io_targets (from_FSM A (t_target tA)) (io'' @ [ioM]) (initial (from_FSM A (t_target tA)))"
        unfolding io_targets.simps using from_FSM_path_initial[OF wf_transition_target[OF \<open>tA \<in> transitions A\<close>]]
        by auto
...
end (* lemma pass_ATC_io_explicit_io_tuple :
  assumes "pass_ATC M A FS"
  and     "is_ATC A"
  and     "observable M"
  and     "(inputs A) \<subseteq> (inputs M)"
...
end (* lemma pass_ATC_io_fail_fixed_io :
  assumes "is_ATC A"
  and     "observable M"
  and     "(inputs A) \<subseteq> (inputs M)"
  and     "io@[ioA] \<in> L A"
...
next
  ...
  next
    case False
    then obtain x where "find (\<lambda> x . \<exists> t \<in> transitions A . t_input t = x \<and> t_source t = initial A) (inputs A) = Some x"
    ...
    obtain tA where "tA \<in> transitions A" and "t_input tA = x" and "t_source tA = initial A"
      using find_condition[OF \<open>find (\<lambda> x . \<exists> t \<in> transitions A . t_input t = x \<and> t_source t = initial A) (inputs A) = Some x\<close>] by blast

    let ?ioA = "(x, t_output tA)"
    let ?ioM = "(x, t_output tM)"

    have "[?ioA] \<in> L A"
      using \<open>tA \<in> transitions A\<close> \<open>t_input tA = x\<close> \<open>t_source tA = initial A\<close> unfolding LS.simps
    proof -
    ...
    (* TODO: extract *)
    have "[?ioM] \<in> L M"
      using \<open>tM \<in> transitions M\<close> \<open>t_input tM = x\<close> \<open>t_source tM = initial M\<close> unfolding LS.simps
    proof -
    ...
    then show ?thesis proof cases
      ...
      proof 
        ...
        qed 
        then obtain t where "p = [t]" and "t \<in> transitions A" and "t_source t = initial A" and "t_input t = x" and "t_output t = t_output tM"
          by auto
        then show "False" 
          using a by blast
      qed
    ...
    next
      ...
      have "(inputs (from_FSM A (t_target t'))) \<subseteq> (inputs (from_FSM M (t_target tM)))"
        using \<open>(inputs A) \<subseteq> (inputs M)\<close> 
      ...
      consider (b1) "initial (from_FSM A (t_target t')) \<in> FS" |
        ...
        using Suc.IH[OF \<open>\<not>(pass_ATC' (from_FSM M (t_target tM)) (from_FSM A (t_target t')) FS k)\<close>
                        is_ATC_from[OF \<open>t' \<in> transitions A\<close> \<open>is_ATC A\<close>]
                        from_FSM_observable[OF \<open>observable M\<close> wf_transition_target[OF \<open>tM \<in> transitions M\<close>]]
        ...
        proof -
          ...
          next
            ...
            moreover have "io_targets A (io @ [ioM]) (t_target t') = io_targets (from_FSM A (t_target t')) (io @ [ioM]) (initial (from_FSM A (t_target t')))"
              unfolding io_targets.simps
              using from_FSM_path_initial[OF wf_transition_target[OF \<open>t' \<in> transitions A\<close>]] by auto
...
proof (rule ccontr)
  ...
  then show "False"
    using pass_ATC_io_fail[OF \<open>\<not> pass_ATC M2 A FS\<close> assms(2,4) \<open>(inputs A) \<subseteq> (inputs M2)\<close>] using assms(1)
  proof -
...
end (* lemma pass_ATC_fail_no_reduction :
  assumes "is_ATC A"
  and     "observable T" 
  and     "observable M"
  and     "(inputs A) \<subseteq> (inputs M)"
...
fun pass_separator_ATC :: "('a,'b,'c) fsm \<Rightarrow> ('d,'b,'c) fsm \<Rightarrow> 'a \<Rightarrow> 'c \<Rightarrow> bool" where
  "pass_separator_ATC M S q1 t2 = pass_ATC (from_FSM M q1) S {t2}"

(*
fun pass_separator_ATC :: "('a,'b,'c) fsm \<Rightarrow> (('a \<times> 'a) + 'a,'b) FSM_scheme \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> bool" where
  "pass_separator_ATC M S q1 q2 = pass_ATC (from_FSM M q1) S {Inr q2}"
*)


end (* lemma separator_is_ATC :
...
(*
end (* lemma state_separator_from_canonical_separator_is_ATC :
...
(* todo: move *)
end (* lemma nodes_initial_deadlock :
...
proof -
  ...
  proof -
    ...
    then have "t \<in> transitions M" and "t_source t = initial M"
...
end (* lemma pass_separator_ATC_from_separator_left :
...
end (* lemma pass_separator_ATC_from_separator_right :
  assumes "observable M"
  and     "q1 \<in> nodes M"
  and     "q2 \<in> nodes M"
  and     "is_separator M q1 q2 A t1 t2" 
...
(*
end (* lemma is_separator_language :
  assumes "is_separator M q1 q2 A t1 t2"
  shows "L A \<subseteq> LS M q1 \<union> LS M q2"


end (* lemma is_separator_target :
  assumes "is_separator M q1 q2 A t1 t2"
...
end (* lemma pass_separator_ATC_path_left :
...
end (* lemma pass_separator_ATC_path_right :
...
end (* lemma pass_separator_ATC_fail_no_reduction :
...
proof 
  ...
  moreover have "pass_ATC (from_FSM S s1) A {t2}"
    ...
    unfolding from_FSM_simps \<open>(inputs S) = (inputs M)\<close> by blast
...
end (* lemma pass_separator_ATC_pass_left :
  assumes "observable S" 
  and     "observable M"
  and     "s1 \<in> nodes S"
  and     "q1 \<in> nodes M"
...
end (* lemma pass_separator_ATC_pass_right :
  assumes "observable S" 
  and     "observable M"
  and     "s2 \<in> nodes S"
  and     "q1 \<in> nodes M"
...
(* TODO: move *)
end (* lemma completely_specified_path_extension : 
  assumes "completely_specified M"
  and     "q \<in> nodes M"
  and     "path M q p"
  and     "x \<in> (inputs M)"
obtains t where "t \<in> transitions M" and "t_input t = x" and "t_source t = target p q"
proof -
  have "target p q \<in> nodes M"
    using path_target_is_node \<open>path M q p\<close> by metis
  then obtain t where "t \<in> transitions M" and "t_input t = x" and "t_source t = target p q"
    using \<open>completely_specified M\<close> \<open>x \<in> (inputs M)\<close> 
    unfolding completely_specified.simps by blast
  then show ?thesis using that by blast
qed

end (* lemma completely_specified_language_extension :
  assumes "completely_specified M"
  and     "q \<in> nodes M"
  and     "io \<in> LS M q"
  and     "x \<in> (inputs M)"
...
proof -
  ...
  moreover obtain t where "t \<in> transitions M" and "t_input t = x" and "t_source t = target p q"
...
end (* lemma language_path_append_transition_observable :
...
end (* lemma pass_separator_ATC_completely_specified_left :
  assumes "observable S" 
  and     "observable M"
  and     "s1 \<in> nodes S"
  and     "q1 \<in> nodes M"
...
end (* lemma pass_separator_ATC_completely_specified_right :
  assumes "observable S" 
  and     "observable M"
  and     "s2 \<in> nodes S"
  and     "q1 \<in> nodes M"
...
end (* lemma pass_separator_ATC_reduction_distinction : 
  assumes "observable M"
  and     "observable S"
  and     "(inputs S) = (inputs M)"
  and     "pass_separator_ATC S A s1 t2"
...
end (* lemma pass_separator_ATC_failure_left : 
  assumes "observable M"
  and     "observable S"
  and     "(inputs S) = (inputs M)"
  and     "is_separator M q1 q2 A t1 t2"
...
end (* lemma pass_separator_ATC_failure_right : 
  assumes "observable M"
  and     "observable S"
  and     "(inputs S) = (inputs M)"
  and     "is_separator M q1 q2 A t1 t2"
...
fun atc_to_io_set :: "('a,'b,'c) fsm \<Rightarrow> ('d,'b,'c) fsm \<Rightarrow> (Input \<times> Output) list set" where
  "atc_to_io_set M A = L M \<inter> L A"


(* very inefficient calculation *)
fun atc_to_io_list :: "('a,'b,'c) fsm \<Rightarrow> ('d,'b,'c) fsm \<Rightarrow> (Input \<times> Output) list list" where
...
end (* lemma acyclic_language_alt_def :
  assumes "acyclic M"
...
definition pass_io_set :: "('a,'b,'c) fsm \<Rightarrow> (Input \<times> Output) list set \<Rightarrow> bool" where
  "pass_io_set M ios = (\<forall> io x y . io@[(x,y)] \<in> ios \<longrightarrow> (\<forall> y' . io@[(x,y')] \<in> L M \<longrightarrow> io@[(x,y')] \<in> ios))"

definition pass_io_set_maximal :: "('a,'b,'c) fsm \<Rightarrow> (Input \<times> Output) list set \<Rightarrow> bool" where
...
end (* lemma pass_io_set_from_pass_io_set_maximal :
...
(* TODO: move *)
end (* lemma finite_set_elem_maximal_extension_ex :
  assumes "xs \<in> S"
  and     "finite S"
...
end (* lemma pass_io_set_maximal_from_pass_io_set :
  assumes "finite ios"
...
(* TODO: move *)
end (* lemma acyclic_observable_maximal_io_sequences :
  assumes "acyclic M"
  and     "observable M"
  ...
  proof -
    ...
    proof -
      ...
      have f4: "\<exists>ps. io = p_io ps \<and> path M (initial M) ps"
        using a1 by simp
      obtain ppsa :: "(integer \<times> integer) list \<Rightarrow> 'a \<Rightarrow> ('a,'b,'c) fsm \<Rightarrow> ('a \<times> integer \<times> integer \<times> 'a) list" where
...
definition atc_to_io_set_maximal :: "('a,'b,'c) fsm \<Rightarrow> ('d,'b,'c) fsm \<Rightarrow> (Input \<times> Output) list set" where
  "atc_to_io_set_maximal M A = {io' \<in> atc_to_io_set M A. \<nexists>io''. io'' \<noteq> [] \<and> io' @ io'' \<in> atc_to_io_set M A}"

fun atc_to_io_list_maximal :: "('a,'b,'c) fsm \<Rightarrow> ('d,'b,'c) fsm \<Rightarrow> (Input \<times> Output) list list" where
  "atc_to_io_list_maximal M A = (let LA = set (map p_io (maximal_acyclic_paths A));
                                     LM = map p_io (paths_up_to_length M (initial M) (size A -1))
                                  in filter (\<lambda>io . io \<in> LA) LM)"

end (* lemma atc_to_io_list_maximal_code :
  assumes "is_ATC A"
...
end (* lemma pass_io_set_from_pass_separator :
  assumes "is_separator M q1 q2 A t1 t2"
  and     "pass_separator_ATC S A s1 t2"
  and     "observable M"
  and     "observable S"
...
proof (rule ccontr) 
  ...
  have "(inputs A) \<subseteq> (inputs (from_FSM S s1))"
    using assms(1) unfolding is_separator_def from_FSM_simps \<open>(inputs S) = (inputs M)\<close> by auto
...
end (* lemma separator_language_last_left :
  assumes "is_separator M q1 q2 A t1 t2"
...
end (* lemma separator_language_last_right :
  assumes "is_separator M q1 q2 A t1 t2"
  and     "completely_specified M"
  and     "q2 \<in> nodes M"
  and     "io @ [(x, y)] \<in> L A"
...
end (* lemma pass_separator_from_pass_io_set :
  assumes "is_separator M q1 q2 A t1 t2"
...
end (* lemma pass_separator_pass_io_set_iff:
  assumes "is_separator M q1 q2 A t1 t2"
  and     "observable M"
  and     "observable S"
  and     "q1 \<in> nodes M"
...
end (* lemma maximal_contained_lists_code[code] :
...
end (* lemma pass_separator_pass_io_set_maximal_iff:
  assumes "is_separator M q1 q2 A t1 t2"
  and     "observable M"
  and     "observable S"
  and     "q1 \<in> nodes M"
