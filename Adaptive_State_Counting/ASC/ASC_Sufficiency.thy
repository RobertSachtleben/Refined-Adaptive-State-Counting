...
TS function constitutes a test suite sufficient to test the FSMs it has been generated for for 
reduction.
...
have been contained in one of the sets calculated by the R function. This is only possible if the
prefix is not a minimal sequence to a failure extending the deterministic state cover or if the
test suite observes a failure, both of which violates the assumptions.
\<close>
...
proof 
  ...
  proof (rule ccontr)
    ...
    \<comment> \<open> show that the prerequisites (Prereq) for LB are met by construction \<close>
...
text \<open>
The following lemmata add to the previous result to show that some FSM M1 is a reduction of FSM M2 
if and only if it is a reduction on the test suite generated by the adaptive state counting 
algorithm for these FSMs.
\<close>
