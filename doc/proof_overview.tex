...
\onslide<+-> {
        ...
        \node [Definition,FSM, right=1cm of V]                            (SizeAssm)          {$|M'| \leq m$};
}
...
\onslide<+->{
        \node [Lemma,ASC_Suite, below=2cm of TS.east, anchor=east]                            (elemTS)          {$LB(xs) \leq m \longrightarrow in\footnotemark(xs) \in TS$};
        \path [line] (TS)         --                              (elemTS);   
}
\onslide<+->{
        \node [Lemma,ASC_Suite, below=1cm of elemTS.north]                            (elemTSmin)          {$minimal(xs) \longrightarrow in(xs) \in TS$};
