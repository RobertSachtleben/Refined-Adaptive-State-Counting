...
\onslide<+->{
        \node [Lemma,diagonal fill={cyan!10}{orange!20},right =0.5cm of mFV_ex]                            (minLB)          {$noRep(xs) \longrightarrow |M'| \geq LB (xs)$ \\ {\tiny \texttt{LB\_count}}};
        \path [line] (LB)         --                              (minLB);   
        \path [line] (ATC)         --                              (minLB);  
}
\onslide<+->{
        \node [Definition,ASC_Suite, right=0.5cm of SizeAssm.east, anchor=west]                            (TS)          {test suite $TS$ \\ {\tiny \textit{fun TS, final\_iteration\_ex}}};
}
\onslide<+->{
...
\centering
    ...
    \only<5-6>{noRep(xs) &= \text{$xs$ does not visit any state of the product machine of $M$ and $M'$ twice}\\}
