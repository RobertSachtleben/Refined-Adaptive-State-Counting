...
module
  FSM(Sum(..), Comm_monoid_add, Nat(..), Num, Set(..), FSM_ext(..), bex, initial,
       ...
       list_as_fun, prefix_pair_tests, preamble_pair_tests, sum,
       d_reachable_states_with_preambles,
       r_distinguishable_state_pairs_with_separators_naive,
       m_traversal_paths_with_witness, preamble_prefix_tests,
...
nodes :: forall a b. (Eq a) => FSM_ext a b -> Set a;
nodes m = Set (nodes_from_distinct_paths m);

size :: forall a b. (Eq a) => FSM_ext a b -> Nat;
size m = card (nodes m);
...
wf_transitions ::
  forall a b. (Eq a) => FSM_ext a b -> [(a, (Integer, (Integer, a)))];
wf_transitions m =
  filter
    (\ t ->
...
observable :: forall a b. (Eq a) => FSM_ext a b -> Bool;
observable m =
  all (\ t1 ->
        all (\ t2 ->
              (if fst t1 == fst t2 &&
...
single_input_by_transition_list ::
  forall a b. (Eq a) => FSM_ext a b -> [(a, (Integer, (Integer, a)))] -> Bool;
...
product ::
  forall a b c d.
    (Eq a, Eq c) => FSM_ext a b -> FSM_ext c d -> FSM_ext (a, c) b;
product a b =
...
completely_specified :: forall a b. (Eq a) => FSM_ext a b -> Bool;
...
d_states_opt :: forall a b. (Eq a) => FSM_ext a b -> Nat -> a -> [(a, Integer)];
d_states_opt m k q =
  d_states
...
s_states_opt :: forall a b. (Eq a) => FSM_ext a b -> Nat -> [(a, Integer)];
s_states_opt c k =
  s_states
...
prefix_pair_testsa ::
  forall a b c.
    (Eq a) => a -> (a -> a -> FSM_ext b c) ->
                     ([(a, (Integer, (Integer, a)))], (Set a, Set a)) ->
                       [[(a, ([(a, (Integer, (Integer, a)))], FSM_ext b c))]];
...
calculate_state_preamble_from_d_states ::
  forall a b. (Eq a) => FSM_ext a b -> a -> Maybe (FSM_ext a b);
...
d_reachable_states_with_preambles ::
  forall a b. (Eq a) => FSM_ext a b -> [(a, FSM_ext a b)];
...
state_separator_from_product_submachine ::
  forall a b.
    (Eq a) => FSM_ext a b -> FSM_ext (a, a) b -> FSM_ext (Sum (a, a) a) b;
...
calculate_state_separator_from_s_states ::
  forall a b.
    (Eq a) => FSM_ext a b -> a -> a -> Maybe (FSM_ext (Sum (a, a) a) b);
...
r_distinguishable_state_pairs_with_separators_naive ::
  forall a b. (Eq a) => FSM_ext a b -> [((a, a), FSM_ext (Sum (a, a) a) b)];
...
r_distinguishable_state_pairs_with_separators ::
  forall a b. (Eq a) => FSM_ext a b -> Set ((a, a), FSM_ext (Sum (a, a) a) b);
...
pairwise_r_distinguishable_state_sets_from_separators ::
  forall a b. (Eq a) => FSM_ext a b -> [Set a];
...
maximal_repetition_sets_from_separators ::
  forall a b. (Eq a) => FSM_ext a b -> [(Set a, Set a)];
