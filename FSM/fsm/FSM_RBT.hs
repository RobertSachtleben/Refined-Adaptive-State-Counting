...
module
  FSM_RBT(Linorder, Sum, FSM_ext(..), Nat, Num, Set(..), m_ex_DR,
           calculate_test_suite, calculate_test_suite_set, size, entries, Rbt(..))
  where {
...
less_list a b = less_eq_list a b && not (a == b);
...
instance (Eq a, Linorder a) => Preorder [a] where {
};

instance (Eq a, Linorder a) => Order [a] where {
};
...
less_eq_FSM_ext ::
  forall a b. (Eq a, Ord a, Ord b) => FSM_ext a b -> FSM_ext a b -> Bool;
less_eq_FSM_ext m1 m2 =
  (if less (initial m1) (initial m2) then True
    else initial m1 == initial m2 &&
           (if less_list (inputs m1) (inputs m2) then True
             else inputs m1 == inputs m2 &&
                    (if less_list (outputs m1) (outputs m2) then True
                      else outputs m1 == outputs m2 &&
                             (if less_list (transitions m1) (transitions m2)
                               then True
                               else transitions m1 == transitions m2 &&
                                      less_eq (more m1) (more m2)))));

equal_FSM_ext :: forall a b. (Eq a, Eq b) => FSM_ext a b -> FSM_ext a b -> Bool;
equal_FSM_ext (FSM_ext initiala inputsa outputsa transitionsa morea)
  (FSM_ext initial inputs outputs transitions more) =
  initiala == initial &&
    inputsa == inputs &&
      outputsa == outputs && transitionsa == transitions && morea == more;

less_FSM_ext ::
  forall a b. (Eq a, Ord a, Eq b, Ord b) => FSM_ext a b -> FSM_ext a b -> Bool;
less_FSM_ext a b = less_eq_FSM_ext a b && not (equal_FSM_ext a b);

instance (Eq a, Ord a, Eq b, Ord b) => Ord (FSM_ext a b) where {
  less_eq = less_eq_FSM_ext;
  less = less_FSM_ext;
};

instance (Eq a, Linorder a, Eq b, Linorder b) => Preorder (FSM_ext a b) where {
};

instance (Eq a, Linorder a, Eq b, Linorder b) => Order (FSM_ext a b) where {
};

instance (Eq a, Linorder a, Eq b, Linorder b) => Linorder (FSM_ext a b) where {
};
...
less_eq_unit :: () -> () -> Bool;
less_eq_unit uu uv = True;

less_unit :: () -> () -> Bool;
less_unit uu uv = False;

instance Ord () where {
  less_eq = less_eq_unit;
  less = less_unit;
};
...
nodes :: forall a. (Eq a, Linorder a) => FSM_ext a () -> Set a;
nodes m = set (nodes_from_distinct_paths m);

size :: forall a. (Eq a, Linorder a) => FSM_ext a () -> Nat;
size m = card (nodes m);
...
wf_transitions ::
  forall a.
    (Eq a, Linorder a) => FSM_ext a () -> [(a, (Integer, (Integer, a)))];
wf_transitions m =
  filter
    (\ t ->
...
deadlock_state :: forall a. (Eq a, Linorder a) => FSM_ext a () -> a -> Bool;
...
product ::
  forall a b.
    (Eq a, Linorder a, Eq b,
      Linorder b) => FSM_ext a () -> FSM_ext b () -> FSM_ext (a, b) ();
product a b =
...
d_states_opt ::
  forall a. (Eq a, Linorder a) => FSM_ext a () -> Nat -> a -> [(a, Integer)];
d_states_opt m k q =
  d_states
...
s_states_opt ::
  forall a. (Eq a, Linorder a) => FSM_ext a () -> Nat -> [(a, Integer)];
s_states_opt c k =
  s_states
...
choose_state_separator_deadlock_input ::
  forall a.
    (Eq a,
      Linorder a) => FSM_ext a () ->
                       FSM_ext (a, a) () -> (a, a) -> Maybe Integer;
...
r_distinguishable_state_pairs_with_separators_naive ::
  forall a.
    (Eq a, Linorder a) => FSM_ext a () -> [((a, a), FSM_ext (Sum (a, a) a) ())];
...
calculate_state_preamble_from_d_states ::
  forall a. (Eq a, Linorder a) => FSM_ext a () -> a -> Maybe (FSM_ext a ());
...
d_reachable_states_with_preambles ::
  forall a. (Eq a, Linorder a) => FSM_ext a () -> [(a, FSM_ext a ())];
